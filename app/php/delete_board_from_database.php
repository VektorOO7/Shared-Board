<?php
    ...
    try {
        ...
        $stmt = $connection->prepare('DELETE FROM shared_boards WHERE board_id = :board_id');
        $stmt->execute(['board_id' => $board_id]);
        ...
        if (is_dir($dir_path)) {
            if (deleteDirectory($dir_path)) {
                echo json_encode(['success' => true, 'message' => 'Board and associated notes, shared boards, and files deleted successfully']);
            } else {
                echo json_encode(['success' => false, 'message' => 'Failed to delete the directory']);
            }
        } else {
            echo json_encode(['success' => true, 'message' => 'Board and associated notes and shared boards deleted, but no directory found']);
        }
